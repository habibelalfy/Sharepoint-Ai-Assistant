/** Live write test: asks the assistant to create project20 through chat. Reads secrets without logging them. */
...
async function main() {
  ...
  try {
    ...
    await page.evaluate(() => {
      messages.push({ role: 'user', content: 'Create a new project' }, { role: 'assistant', content: 'I cannot create a new project. There is no project-creation tool.' });
    });
    const started = new Date().toISOString();
    await page.locator('#input').fill('craete new project name project20');
    ...
    const projects = (await result.json()).result.filter(
      (p) => p.title.toLowerCase() === 'project20',
    );
    assert.equal(projects.length, 1, 'Expected exactly one project20 project');
    ...
    await page.screenshot({ path: 'plans/e2e-project20.png', fullPage: true });
    const report = {
      passed: true,
      project: projects[0],
      reply: data.reply,
    ...
    writeFileSync('plans/e2e-project20-result.json', JSON.stringify(report, null, 2) + '\n');
